(function() {
	...
	if (missingGenes.length > 0)
	    console.warn ("Warning: the following genes were not found in the associations list: " + missingGenes)
	...
	if (conf.terms)
        ...
        // excluding those which are indistinguishable from their children
        var relevantTerms = util.removeDups (geneSet.reduce (function(termList,g) {
	    return termList.concat (assocs.termsByGene[g])
	}, [])).sort(util.numCmp).filter (function (term) {
            return assocs.ontology.children[term].length == 0
                || assocs.ontology.children[term].some (function (child) {
                    return !util.arraysEqual (assocs.genesByTerm[term], assocs.genesByTerm[child])
                })
        })
