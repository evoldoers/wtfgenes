...
set<BernouilliParamIndex> BernouilliParamCounts::combinedIndices (const BernouilliParamCounts& other) const {
  set<BernouilliParamIndex> idx = allIndices(), otherIdx = other.allIndices();
  idx.insert (otherIdx.begin(), otherIdx.end());
  return idx;
}

set<BernouilliParamIndex> BernouilliParamCounts::allIndices() const {
...
LogProb BernouilliParamCounts::logBetaBernouilli (const BernouilliParamCounts& prior) const {
...
LogProb BernouilliParamCounts::deltaLogBetaBernouilli (const BernouilliParamCounts& delta) const {
...
BernouilliParamCounts& BernouilliParamCounts::operator+= (const BernouilliParamCounts& c) {
  for (auto pc : c.succ)
