(function() {
	...
	mcmc.summaryCallback.push (function (mcmc, summ) {
	    ...
	    mcmc.models.forEach (function (model, m) {
		var nTerms = model.relevantTerms.length
		...
		var termProbSqNorm = termProb.map (function (p) { return 1 / (p*p) })
		var termAuto = {}
		points.forEach (function(tau) {
			...
			var sum = util.sumList (commonTerms.map (function (term) { return termProbSqNorm[term] }))
			R_tau.push (sum)
		    }
		    termAuto[tau] = jStat.mean(R_tau) / nTerms - 1
		})
