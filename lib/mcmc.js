(function() {
	...
	for (var sample = 0; sample < samples; ++sample) {
	    ...
	    var mr = moveTypes.map(function(t){return moveRate[t]})
	    var move = { sample: sample,
			 totalSamples: samples,
			 type: moveTypes [util.randomIndex (mr, mcmc.generator)] }

//	    console.warn ("mr=" + mr + " move.type = " + move.type)

	    switch (move.type) {
	    case 'flip':
		move.model = mcmc.models [util.randomIndex (mcmc.modelWeight, mcmc.generator)]
		extend (move, move.model.proposeFlipMove.bind(move.model) ())
		move.model.sampleMoveCollapsed (move, mcmc.countsWithPrior)
		break

	    case 'swap':
		move.model = mcmc.models [util.randomIndex (nActiveTerms, mcmc.generator)]
